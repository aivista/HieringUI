...
export class ProfileDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    this.candidateDetails = this.candidateService.getData('candidateServiceDetails');
    ...
    if (this.candidateDetails && this.candidateDetails.email) {
      ...
      this.candidateService.CandidateDetails({ email: this.candidateDetails.email }).subscribe(
        (response: any) => {
          console.log('API Response:', response);
          const ProfileDetails = response.result;
          // Bind API response to profile
          this.profile = {
            name: `${ProfileDetails.title || ''} ${ProfileDetails.firstName || ''} ${ProfileDetails.middleName || ''} ${ProfileDetails.lastName || ''}`.trim(),
            imageUrl: ProfileDetails.imageUrl || 'assets/icons/profileLogo.svg', 
            certifications: (ProfileDetails.certification || '').split(','),
            email: ProfileDetails.email || '',
            phone: ProfileDetails.contact || '',
            location: ProfileDetails.address || '',
            latestExperience: {
              company: ProfileDetails.latestExperience?.company || 'Nxtra Data Ltd. (A Bharti Airtel Group Company)',
              employmentType: ProfileDetails.latestExperience?.employmentType || 'Full Time',
              period: ProfileDetails.latestExperience?.period || 'September, 2022-Present',
              location: ProfileDetails.latestExperience?.location || 'Mumbai, Maharashtra',
              position: ProfileDetails.designation|| '',
            },
            latestEducation: {
              university: ProfileDetails.latestEducation?.university || 'Pune University, Maharashtra',
              degree: ProfileDetails.education || '',
              major: ProfileDetails.major || 'Computer Science',
              period: ProfileDetails.latestEducation?.period || 'September, 2022-Present',
              activities: ProfileDetails.latestEducation?.activities || 'football team head',
            },
